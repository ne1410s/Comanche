...
# Run unit tests
gci -r -dir ../TestResults | % { rm -r $_ }; dotnet test -c Release --coverlet --coverlet-output-format cobertura --coverlet-include "[Comanche]*"; dotnet reportgenerator -targetdir:coveragereport -reports:**/coverage.cobertura.*.xml -reporttypes:"html;jsonsummary"; start coveragereport/index.html;
